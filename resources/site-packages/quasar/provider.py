...
def parse_rss(root):
    ...
    def _text(node, pathname):
        n = node.find(pathname)
        if n is not None:
            return n.text

    def _attr(node, pathname, attrib):
        n = node.find(pathname)
        if n is not None:
    ...
    for item in root.getiterator("item"):
        yield {
            ...
            "uri": _text(item,
                         ".//{%(torrent)s}magnetURI" % nsmap) or _attr(item,
                                                                       ".//enclosure", "url") or _text(item, "./link"),
            "seeds": _maybe_int(_text(item, ".//{%(torrent)s}seeds" % nsmap) or
                                _text(item, "numSeeders") or
                                _text(item, "seeders")),
            "peers": _maybe_int(_text(item, ".//{%(torrent)s}peers" % nsmap) or
                                _text(item, "numLeechers") or
                                _text(item, "leechers")),
            "size": _maybe_int(_text(item, ".//{%(torrent)s}contentLength" % nsmap) or
                               _attr(item, ".//enclosure", "length")),
        }
...
def generate_payload(generator=None, read_magnet_link=False):
    Filtering.information()  # print filters settings
    results = []
    cont = 0
